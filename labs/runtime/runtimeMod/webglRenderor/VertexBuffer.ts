...
export class VertexBuffer {
    ...
    nVBLength: number = 0;
    nVertNum: number = 0;
    memBuffer: Array<membuf.MemClass> = null;
    ...
    create(stride: number, vertNum: number): Boolean {
        ...
        this.nStreamNum = 1;
        this.memBuffer = new Array<membuf.MemClass>(1);
    ...
    createMS(vertNum: number, stride: Array<number>): boolean {
        ...
        this.stride = stride;
        this.memBuffer = new Array<membuf.MemClass>(this.nStreamNum);
    ...
    getMemBuffer(id: number): membuf.MemClass {
    ...
    upload(gl: WebGLRenderingContext, target: number): void {
        if (this.nStreamNum <= 0)
            return;
        var membuf: membuf.MemClass = null;
        var bufsz: number = 0;
        if (!this.vbo) {
